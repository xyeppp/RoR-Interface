MagusResource = PetWindow:Subclass ("PetWindow")

CareerResource:RegisterResource (GameData.CareerLine.MAGUS, MagusResource)

-- The "portrait" will set its background to one of these slices
local PetStateTextures =
{
    MAIN_TEXTURE            = "EA_Career_Ma_32b",
...
function MagusResource:Create (windowName)
    local frame = self:ParentCreate (windowName, PetStateTextures, PetCommands)
    
    if (frame)
    then
        frame.m_Data = CareerResourceData:Create (GameData.CareerLine.MAGUS)
...
function MagusResource:UpdateResourceDisplay (previousResourceValue, currentResourceValue)
