EngineerResource = PetWindow:Subclass ("PetWindow")

CareerResource:RegisterResource (GameData.CareerLine.ENGINEER, EngineerResource)

-- The "portrait" will set its background to one of these slices
local PetStateTextures =
{
    MAIN_TEXTURE            = "EA_Career_En_32b",
...
function EngineerResource:Create (windowName)
    local frame = self:ParentCreate (windowName, PetStateTextures, PetCommands)
    
    if (frame)
    then
        frame.m_Data = CareerResourceData:Create (GameData.CareerLine.ENGINEER)
...
function EngineerResource:UpdateResourceDisplay (previousResourceValue, currentResourceValue)
